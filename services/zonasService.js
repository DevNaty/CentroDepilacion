...
class ZonasService {
    async getAllZonas() {
        try {
            const result = await sql.query('SELECT ID_Zona, Nombre_Zona FROM Zonas');
    ...
    async getZonaById(id) {
        try {
            const result = await sql.query`SELECT ID_Zona, Nombre_Zona FROM Zonas WHERE ID_Zona = ${id}`;
    ...
    async createZona(zona) {
        try {
            ...
            `;
            return { ID_Zona: result.recordset[0].ID_Zona, ...zona };
        } catch (err) {
    ...
    async deleteZona(id) {
        try {
            const result = await sql.query`DELETE FROM Zonas WHERE ID_Zona = ${id}`;
